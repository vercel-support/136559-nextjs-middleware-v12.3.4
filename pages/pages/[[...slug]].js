    ...
    <div className={styles.container}>
      ...
      <main className={styles.main}>
        ...
        <div style={{ textAlign: 'left', width: '100%' }}>
          router.isFallback:
          <pre>{router.isFallback ? `true` : `false`}</pre>
        </div>

        <div style={{ textAlign: 'left', width: '100%' }}>
          router.query:
          <pre>{router ? JSON.stringify(router.query) : ''}</pre>
        </div>

        <div style={{ textAlign: 'left', width: '100%' }}>
          router.asPath:
          <pre>{router ? router.asPath : ''}</pre>
        </div>
      </main>
    </div>
  );
...
export async function getStaticProps({ params }) {
  ...
  console.log(
    `${slug} - timestamp returned from https://nextjs-time-api.vercel-support.app/api/time: ${now}`
  );
